%只考虑连杆质量不考虑滑块的
%% 参数
%
g=-9.8;
m1=1;l1=1;J1=m1*l1^2 /3;
m2=1;l2=2;J2=m2*l2^2 /3;
%%
A = diag([m1,m1,J1,m2,m2,J2]);
B = [0; m1*g; 10; 0;m2*g;0];
q0 = [l1; 0; 0; 2*l1+l2; 0; 0];
v0 = [0; 0; 0; 0; 0; 0]; %初始状态
q=q0;v=v0;
phi = [ q0(1) - l1*cos(q0(3));
    q0(2) - l1*sin(q0(3));
    -2*l1*cos(q0(3))+q0(4)-l2*cos(q0(6));
    -2*l1*sin(q0(3))+q0(5)-l2*sin(q0(6));
    -2*l1*sin(q0(3))+2*l2*sin(q0(6))]; %约束
phiq = [1 0 l1*sin(q0(3)) 0 0 0;
    0 1 -l1*cos(q0(3)) 0 0 0;
    0 0 2*l1*sin(q0(3)) 1 0 l2*sin(q0(6));
    0 0 -2*l1*cos(q0(3)) 0 1 -l2*cos(q0(6));
    0 0 2*l1*cos(q0(3)) 0 0 2*l2*cos(q0(6))]; %雅可比
P=[-l1*cos(q0(3))*v0(3)^2;
    -l1*sin(q0(3))*v0(3)^2;
    -2*l1*cos(q0(3))*v0(3)^2-l2*cos(q0(6))*v0(6)^2;
    -2*l1*sin(q0(3))*v0(3)^2-l2*sin(q0(6))*v0(6)^2;
    2*l1*sin(q0(3))*v0(3)^2+2*l2*sin(q0(6))*v0(6)^2];
h = 0.001; T = 5; t = 0:h:T; %步长及时间


phiT = phiq * v; %对时间全微分
alpha = 25; beta = 50; %经验值5~50
P1 = P - 2 * alpha * phiT - beta ^ 2 * phi; %稳定形式
LEFT = [A phiq'; phiq zeros(length(P))]; %左端系数
RIGHT = [B; P1]; %右端
X = (LEFT ^ -1) * RIGHT;

%% 组装
q=q0;v=v0;
column = zeros(6, (length(t) - 1));
q = [q column]; v = [v column];
a=zeros(6,length(t));
%%
for i =1:length(t)-1
    phi = [ q(1,i) - l1*cos(q(3,i));
        q(2,i) - l1*sin(q(3,i));
        -2*l1*cos(q(3,i))+q(4,i)-l2*cos(q(6,i));
        -2*l1*sin(q(3,i))+q(5,i)-l2*sin(q(6,i));
        -2*l1*sin(q(3,i))+2*l2*sin(q(6,i))]; %约束
    phiq = [1 0 l1*sin(q(3,i)) 0 0 0;
        0 1 -l1*cos(q(3,i)) 0 0 0;
        0 0 2*l1*sin(q(3,i)) 1 0 l2*sin(q(6,i));
        0 0 -2*l1*cos(q(3,i)) 0 1 -l2*cos(q(6,i));
        0 0 2*l1*cos(q(3,i)) 0 0 2*l2*cos(q(6,i))]; %雅可比
    P=[-l1*cos(q(3,i))*v(3,i)^2;
        -l1*sin(q(3,i))*v(3,i)^2;
        -2*l1*cos(q(3,i))*v(3,i)^2-l2*cos(q(6,i))*v(6,i)^2;
        -2*l1*sin(q(3,i))*v(3,i)^2-l2*sin(q(6,i))*v(6,i)^2;
        2*l1*sin(q(3,i))*v(3,i)^2+2*l2*sin(q(6,i))*v(6,i)^2];
    phiT = phiq * v(:, i);
    P1 = P - 2 * alpha * phiT - (beta ^ 2) * phi;
    LEFT = [A phiq'; phiq zeros(length(P))];
    RIGHT = [B; P1];
    X = (LEFT ^ -1) * RIGHT;
    a(:,i)=X(1:6);
    v(:, i + 1) = v(:, i) + h * a(:, i);
    q(:, i + 1) = q(:, i) + h * v(:, i);
end
%% figure
plot(t,q(3,:));
